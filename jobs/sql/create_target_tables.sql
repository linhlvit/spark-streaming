-- ============================================================
-- Tạo 3 bảng TARGET trong schema FSS_STREAM
-- Cấu trúc 1:1 với bảng nguồn, không có constraint CHECK/FK
-- ============================================================

-- ── T24_ACCOUNT_TARGET ──────────────────────────────────────
CREATE TABLE FSS_STREAM.T24_ACCOUNT_TARGET (
...
-- ── T24_CUSTOMER_TARGET ─────────────────────────────────────
CREATE TABLE FSS_STREAM.T24_CUSTOMER_TARGET (
...
CREATE TABLE FSS_STREAM.T24_TRANSACTIONS_TARGET (
...
-- ── T24_BRANCH_TARGET ───────────────────────────────────────
CREATE TABLE FSS_STREAM.T24_BRANCH_TARGET (
    BRANCH_CODE         VARCHAR2(20),
...
CREATE TABLE FSS_STREAM.T24_TXN_PENDING_JOIN (
    TRANSACTION_ID      VARCHAR2(50),
    ACCOUNT_ID          VARCHAR2(50)        NOT NULL,
    CUSTOMER_ID         VARCHAR2(50)        NOT NULL,
    TRANSACTION_DATE    DATE                NOT NULL,
...
-- ALTER TABLE FSS_STREAM.T24_TXN_PENDING_JOIN ADD (
--     STATUS        VARCHAR2(20)  DEFAULT 'PENDING',
--     ERROR_CODE    VARCHAR2(50),
--     ERROR_REASON  VARCHAR2(200),
--     NEXT_RETRY_AT TIMESTAMP     DEFAULT SYSTIMESTAMP,
...
-- ── T24_TXN_ENRICHED ────────────────────────────────────────
CREATE TABLE FSS_STREAM.T24_TXN_ENRICHED (
    TRANSACTION_ID      VARCHAR2(50),
    ACCOUNT_ID          VARCHAR2(50)        NOT NULL,
    CUSTOMER_ID         VARCHAR2(50)        NOT NULL,
    TRANSACTION_DATE    DATE                NOT NULL,
...
CREATE TABLE FSS_STREAM.T24_TXN_ACCOUNT_SNAPSHOT (
    TRANSACTION_ID          VARCHAR2(50),
    ACCOUNT_ID              VARCHAR2(50)        NOT NULL,
    CUSTOMER_ID             VARCHAR2(50)        NOT NULL,
    TRANSACTION_DATE        DATE                NOT NULL,
    ...
    BALANCE_IS_APPROXIMATE  NUMBER(1)           DEFAULT 0,
...
CREATE TABLE FSS_STREAM.T24_TXN_ACCT_PENDING_JOIN (
    TRANSACTION_ID      VARCHAR2(50),
    ACCOUNT_ID          VARCHAR2(50)        NOT NULL,
    CUSTOMER_ID         VARCHAR2(50)        NOT NULL,
    TRANSACTION_DATE    DATE                NOT NULL,
...
-- Migration cho DB đang chạy (chạy nếu bảng đã tồn tại):
-- ALTER TABLE FSS_STREAM.T24_TXN_ACCT_PENDING_JOIN ADD (
--     ERROR_CODE    VARCHAR2(50),
--     NEXT_RETRY_AT TIMESTAMP DEFAULT SYSTIMESTAMP
-- );
...
CREATE TABLE FSS_STREAM.T24_BRANCH_SALES_SUMMARY (
    BRANCH_CODE     VARCHAR2(20)        NOT NULL,
...
CREATE INDEX IDX_BRANCH_SALES_RPT_DATE ON FSS_STREAM.T24_BRANCH_SALES_SUMMARY (RPT_DATE, BRANCH_CODE);
...
CREATE TABLE FSS_STREAM.T24_STREAM_BATCH_LOG (
...
-- CDC Cache Join: T24_TRANSACTIONS ⋈ _customer_cache (CDC-maintained)
-- Thêm CUSTOMER_NAME, SEGMENT so với T24_TXN_ENRICHED (branch join)
CREATE TABLE FSS_STREAM.T24_TXN_CUSTOMER_ENRICHED (
    TRANSACTION_ID      VARCHAR2(50),
    ACCOUNT_ID          VARCHAR2(50)        NOT NULL,
    CUSTOMER_ID         VARCHAR2(50)        NOT NULL,
    TRANSACTION_DATE    DATE                NOT NULL,
    ...
    TRANSACTION_STATUS  VARCHAR2(20),
    ENRICHED_AT         TIMESTAMP           DEFAULT SYSTIMESTAMP,
    CONSTRAINT PK_TXN_CUSTOMER_ENRICHED PRIMARY KEY (TRANSACTION_ID)
);

-- ── T24_TXN_CUSTOMER_PENDING (DLQ) ──────────────────────────
-- Flat schema: lưu toàn bộ TXN payload để retry không cần đọc lại Kafka.
-- STATUS    lifecycle: PENDING → RESOLVED (join thành công) | FAILED (hết MAX_RETRY)
-- ERROR_CODE: CUSTOMER_NOT_FOUND | CUSTOMER_CACHE_WARMING
-- ERROR_REASON: free-text chi tiết (customer_id, context)
-- RESOLVED_AT: timestamp khi retry thành công hoặc mark FAILED
CREATE TABLE FSS_STREAM.T24_TXN_CUSTOMER_PENDING (
    TRANSACTION_ID      VARCHAR2(50),
    ACCOUNT_ID          VARCHAR2(50)        NOT NULL,
    CUSTOMER_ID         VARCHAR2(50)        NOT NULL,
    TRANSACTION_DATE    DATE                NOT NULL,
    ...
    ERROR_REASON        VARCHAR2(200),
    RESOLVED_AT         TIMESTAMP,
    CONSTRAINT PK_TXN_CUSTOMER_PENDING PRIMARY KEY (TRANSACTION_ID)
);
