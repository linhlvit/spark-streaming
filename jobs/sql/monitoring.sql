...
-- ============================================================
SELECT 'T24_TRANSACTIONS_TARGET'   AS tbl, COUNT(*) AS rows FROM LPB_POC.T24_TRANSACTIONS_TARGET
UNION ALL
SELECT 'T24_ACCOUNT_TARGET',        COUNT(*) FROM LPB_POC.T24_ACCOUNT_TARGET
UNION ALL
SELECT 'T24_CUSTOMER_TARGET',       COUNT(*) FROM LPB_POC.T24_CUSTOMER_TARGET
UNION ALL
SELECT 'T24_BRANCH_TARGET',         COUNT(*) FROM LPB_POC.T24_BRANCH_TARGET
UNION ALL
SELECT '--- enriched ---',          0        FROM DUAL
UNION ALL
SELECT 'T24_TXN_ENRICHED',          COUNT(*) FROM LPB_POC.T24_TXN_ENRICHED
UNION ALL
SELECT 'T24_TXN_ACCOUNT_SNAPSHOT',  COUNT(*) FROM LPB_POC.T24_TXN_ACCOUNT_SNAPSHOT
UNION ALL
SELECT 'T24_BRANCH_SALES_SUMMARY',  COUNT(*) FROM LPB_POC.T24_BRANCH_SALES_SUMMARY
UNION ALL
SELECT '--- dlq ---',               0        FROM DUAL
UNION ALL
SELECT 'DLQ_TXN_PENDING_JOIN',      COUNT(*) FROM LPB_POC.T24_TXN_PENDING_JOIN       WHERE STATUS = 'PENDING'
UNION ALL
SELECT 'DLQ_ACCT_PENDING_JOIN',     COUNT(*) FROM LPB_POC.T24_TXN_ACCT_PENDING_JOIN  WHERE STATUS = 'PENDING'
ORDER BY 1;
...
SELECT
    ...
    UPDATED_AT
FROM LPB_POC.T24_BRANCH_SALES_SUMMARY
...
FROM (
    SELECT
        (SELECT COUNT(*) FROM LPB_POC.T24_TRANSACTIONS_TARGET
         WHERE TRANSACTION_DATE >= TRUNC(SYSDATE))           AS total_txn,
        (SELECT COUNT(*) FROM LPB_POC.T24_TXN_ENRICHED
         WHERE ENRICHED_AT >= TRUNC(SYSDATE))                AS enriched_ok,
        (SELECT COUNT(*) FROM LPB_POC.T24_TXN_ACCOUNT_SNAPSHOT
         WHERE SNAPSHOT_AT >= TRUNC(SYSDATE))                AS snapshot_ok,
        (SELECT COUNT(*) FROM LPB_POC.T24_TXN_PENDING_JOIN
         WHERE STATUS = 'PENDING'
           AND PENDING_SINCE >= TRUNC(SYSDATE))              AS dlq_branch,
        (SELECT COUNT(*) FROM LPB_POC.T24_TXN_ACCT_PENDING_JOIN
         WHERE STATUS = 'PENDING'
...
FROM LPB_POC.T24_TRANSACTIONS_TARGET src
WHERE NOT EXISTS (
    SELECT 1 FROM LPB_POC.T24_TRANSACTIONS_TARGET tgt
    WHERE tgt.TRANSACTION_ID = src.TRANSACTION_ID
);
-- Lưu ý: nếu source và target cùng schema thì dùng bảng nguồn Oracle thực tế
-- SELECT COUNT(*) FROM LPB_POC.T24_TRANSACTIONS src
-- WHERE NOT EXISTS (SELECT 1 FROM LPB_POC.T24_TRANSACTIONS_TARGET tgt
--                   WHERE tgt.TRANSACTION_ID = src.TRANSACTION_ID);
...
FROM LPB_POC.T24_TRANSACTIONS_TARGET t
LEFT JOIN LPB_POC.T24_TXN_ENRICHED          e  ON e.TRANSACTION_ID = t.TRANSACTION_ID
LEFT JOIN LPB_POC.T24_TXN_ACCOUNT_SNAPSHOT  s  ON s.TRANSACTION_ID = t.TRANSACTION_ID
LEFT JOIN LPB_POC.T24_TXN_PENDING_JOIN      p1 ON p1.TRANSACTION_ID = t.TRANSACTION_ID
LEFT JOIN LPB_POC.T24_TXN_ACCT_PENDING_JOIN p2 ON p2.TRANSACTION_ID = t.TRANSACTION_ID
